...
tweets_output = "output_Vader.csv"

#columns of the csv file
cols = ['id','created_at', 'source', 'original_text','clean_text', 'sentiment','polarity','favorite_count', 'retweet_count', 'possibly_sensitive', 'hashtags',
'user_mentions', 'place']
...
def extract_tweets(keyword, file):
    ...
    for page in tweepy.Cursor(api.search, q=keyword, count = 5, include_rts=False, since=start_date).pages(50):
        for status in page:
            ...
            #Vader for Sentiment Calculation
            ...
            new_entry += [status['id'], status['created_at'], status['source'], status['text'],
                          clean_text, sentiment, polarity, status['favorite_count'], status['retweet_count']]
            try:
